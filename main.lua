...
function love.load()
	tree_manager:index(enemies_manager:set_enemie(nil, { 20, 20, 0 }, 1, 1, { type = "c", radius = 20 }), "enemies")
...
function love.update(dt)
	...
	if love.mouse.isDown(1) then
		local _pos = love.mouse.getPosition()
		enemies_manager:set_enemie(nil, _pos, 20, 0, { type = "c", radius = "20" })
	end
	...
	tree_manager:update_w_matrix(tree_manager)
end
